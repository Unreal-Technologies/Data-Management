...
namespace Shared.Controls
{
        ...
        where Tenum : Enum
    {
        ...
        protected void WriteUserLog<Tentity>(string text, Tentity entity, History.States state)
            where Tentity : class
        {
            User? user = ApplicationState.AuthenticatedUser;
            if(user == null)
            {
                return;
            }
            ...
            History history = new()
            {
                Data = Serializer<Tentity>.Serialize(entity),
                State = state,
                Log = log,
                Table = typeof(Tentity).AssemblyQualifiedName,
            };
